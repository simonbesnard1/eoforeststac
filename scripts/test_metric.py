    ...
    for _, tr in towers_px.iterrows():
        ...
        if Bp_f.size >= 10:
            ...
            try:
                ...
                Dm = (
                    float(np.sqrt(d.T @ invS @ d)) if np.all(np.isfinite(d)) else np.nan
                )
...
def biomass_trend_ols(B: xr.DataArray) -> xr.DataArray:
    ...
    if np.issubdtype(B.time.dtype, np.datetime64):
        years = xr.DataArray(
            B["time"].dt.year.astype(np.float32), dims=["time"], coords={"time": B.time}
        )
    else:
        years = xr.DataArray(
            B["time"].astype(np.float32), dims=["time"], coords={"time": B.time}
        )
...
def height_site_index(
    H: xr.DataArray, ref_year: int = 2020, alpha_years: float = 10.0
) -> xr.Dataset:
    """
...
def _open_crop_da(da, bbox):
    ...
    lon_name = (
        "lon" if "lon" in coords else ("longitude" if "longitude" in coords else None)
    )
    lat_name = (
        "lat" if "lat" in coords else ("latitude" if "latitude" in coords else None)
    )
...
def site_1d_mahalanobis_BH(
    ...
    for p in roi_paths:
        ...
        B_core_vals = _mask_by_polygon(
            _open_crop_da(B2020, _bbox(geom_core)), geom_core
        )
        H_core_vals = _mask_by_polygon(
            _open_crop_da(H2020, _bbox(geom_core)), geom_core
        )
        ...
        rows.append(
            {
                ...
                "B_q95_ctx": muB,
                "B_sigma_ctx": sigB,
                "n_ctx_B": int(b_ctx.size),
                "H_q95_ctx": muH,
                "H_sigma_ctx": sigH,
                "n_ctx_H": int(h_ctx.size),
...
# 1) Load rasters
height = provider.open_dataset(collection_id="POTAPOV_HEIGHT", version="1.0")
# height = subset(
#     height,
#     geometry=geometry)["canopy_height"]

# Biomass time series: choose one source
B = provider.open_dataset(collection_id="CCI_BIOMASS", version="6.0")
# B = subset(
#     B,
...
roi_paths = sorted(
    glob.glob(
        "/home/simon/Documents/science/GFZ/projects/foreststrucflux/data/geojson/*.geojson"
    )
)
...
years = xr.DataArray(
    Bp.time.values.astype(np.int32), dims=["time"], coords={"time": Bp.time}
)
...
H_tex_std = local_texture_std(height_p.rename("H"), radius_m=1000).rename(
    "H_tex_std_1km"
)
...
B_def.to_dataset(name="B_deficit").to_zarr(
    os.path.join(CFG["out_dir"], "B_deficit.zarr"), mode="w"
)
