...
ds_biomass = subset(B, geometry=geometry)  # optional
...
df_dm = site_1d_mahalanobis_BH(
    roi_paths=roi_paths, B2020=B.sel(time="2020-01-01")["aboveground_biomass"]
)
...
ax.set_title(
    "Mahalanobis distance across FLUXNET sites", fontsize=15, fontweight="bold"
)

cbar = plt.colorbar(sc, ax=ax)
cbar.set_label("Biomass potential (q95, 5 km) [MgC ha$^{-1}$]", fontsize=12)
...
ax.hist(df_dm["B_MD_median"], bins=25, color="#4C72B0", alpha=0.85)

ax.set_xlabel("Mahalanobis distance to steady state", fontsize=13)
ax.set_ylabel("Number of sites", fontsize=13)
ax.set_title(
    "Most ecosystems are far from steady state", fontsize=15, fontweight="bold"
)
...
plt.savefig(
    "/home/simon/Documents/science/GFZ/presentation/fluxcom-x/kickoff2026_besnard/images/data_mahalanobis",
    dpi=220,
    bbox_inches="tight",
)
